...
// 여러번 계산하지 않아도된다.. 계산한 값을 그냥 cache에 적립.
// 이미 계산된 값을 어딘가 저장해두고. 필요할때마다 꺼내쓴다. 값을 부를때 memoFibo에 저장된 값이면 바로 호출
// 없으면 if문 위에거 돌고 계산해준다..
// 그냥 const cache = {} 라고 만들어도 되지만. 이미 전역에 노출된 상태 (오염) 그리고 3000줄밑에 또 쓰일 수도 있다.
// 따라서 memoFibo.cache로 하면 함수 내에서만 접근가능한 객체가 생성이 되는 것이다.
const memoFibo = (n) => {
  if (n <= 0) return 0;
  if (n <= 2) return 1;
  if (memoFibo.dir[n]) {
...
// 함수가 있는데 cache를 붙여준것. 여기에 저장해놓고 필요할때마다 함수에서 가져다 쓰는 것
memoFibo.cache = {
  1: 10,
  2: 30,
};
