...
const num = [10, -4, 3, 90, 101, 234, -10000];

// ~ 보다 큼 또는 작음 → boolean
let lessThen = num[0] < num[3]; //true
let greaterThen = num[0] > num[3]; //false

// ~ 보다 크거나 작음 또는 같음 → boolean
let lessThenAndEqual = num[0] <= num[3];
let greaterThenAndEqual = num[0] >= num[3]; //false

// 동등(등급이나 정도가 같다) → boolean
let equal = num[0] == num[3]; //false

// 다름(같지 않음) → boolean
let inequal = num[0] != num[3];

// 동일(똑같다) → boolean
let same = num[0] === num[3];
